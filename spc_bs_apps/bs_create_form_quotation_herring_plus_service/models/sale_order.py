...
class SaleOrder(models.Model):
    ...
    def _get_herring_plus_service_other_lines(self):
        ...
        for raw_line in text.splitlines() or [""]:
            lines.extend(textwrap.wrap(raw_line, width=HERRING_PLUS_SERVICE_OTHER_CHARS_PER_LINE) or [""])
    ...
    def _get_herring_plus_service_footer_page_budget(self, page_index, total_pages):
        ...
        """
        budget = HERRING_PLUS_SERVICE_FOOTER_LINES_PER_PAGE
        if page_index == 0:
            budget -= HERRING_PLUS_SERVICE_FOOTER_INTRO_RESERVED_LINES
        return max(1, budget)
    ...
    def _get_herring_plus_service_total_page_count(self):
        ...
        ``_get_herring_plus_service_footer_page_budget``) before overflowing onto
        ...
        """
        self.ensure_one()
        lines = self._get_herring_plus_service_other_lines()
        pages = 1
        while True:
            capacity = sum(
                self._get_herring_plus_service_footer_page_budget(i, pages) for i in range(pages)
            )
            if len(lines) <= capacity:
                return pages
            pages += 1

    def _get_herring_plus_service_other_text(self, page_index):
        """Portion of the note text belonging on ``page_index`` (0-based).
        Real text fills each page up to its own budget from
        ``_get_herring_plus_service_footer_page_budget``, then pads with blank
        lines up to that same budget.
        """
        self.ensure_one()
        lines = self._get_herring_plus_service_other_lines()
        total_pages = self._get_herring_plus_service_total_page_count()
        start = sum(
            self._get_herring_plus_service_footer_page_budget(i, total_pages) for i in range(page_index)
        )
        real_budget = self._get_herring_plus_service_footer_page_budget(page_index, total_pages)
