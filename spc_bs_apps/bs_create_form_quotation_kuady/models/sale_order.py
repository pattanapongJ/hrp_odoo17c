...
# text at 24px AngsanaNew across the report's full content width (~190mm,
# i.e. A4 minus the 6mm side margins). First-pass estimate scaled from the
# article table's own 40%-wide Description column (~50 chars at that
# width, see report_main_content.xml) - NOT calibrated against a real
# print; re-check against an actual long-text quotation PDF and adjust if
# lines wrap noticeably short/long.
KUADY_OTHER_CHARS_PER_LINE = 120
# Total printed lines available in the footer per page, at 24px AngsanaNew
# over the report's 108mm bottom margin (see
# paper_format_bs_create_quotation_kuady_report). First-pass estimate;
# re-check against a real print.
KUADY_FOOTER_LINES_PER_PAGE = 12
# Lines reserved out of the first page's budget for the "Above price..."
# boilerplate line (rendered as ``.first-page`` in the footer template) -
# real other-template text only starts after it.
KUADY_FOOTER_INTRO_RESERVED_LINES = 2
# Lines reserved out of the last page's budget for the TOTAL PRICE row and
# the signature block (both rendered as ``.last-page``). If the first page
# also ends up being the last page, both reservations apply together.
KUADY_FOOTER_SIGNATURE_RESERVED_LINES = 6

# Characters that fit on one wrapped line of the article table's own
# Description column (~40% of the report's ~190mm content width) at 24px
# AngsanaNew - matches what's actually observed wrapping in that column.
KUADY_ARTICLE_DESC_CHARS_PER_LINE = 50
...
# the 105mm top / 108mm bottom margins). First-pass estimate; re-check
# against a real print with many/long order lines.
KUADY_ARTICLE_LINE_SLOTS_PER_PAGE = 9
...
class SaleOrder(models.Model):
    ...
    def _get_kuady_other_lines(self):
        ...
        for raw_line in text.splitlines():
            # html2plaintext inserts blank lines between paragraphs/list
            # items - keeping them here would show up as uneven gaps
            # between some lines but not others once wrapped and printed.
            if not raw_line.strip():
                continue
            lines.extend(textwrap.wrap(raw_line, width=KUADY_OTHER_CHARS_PER_LINE) or [""])
        return lines
    ...
    def _get_kuady_article_page_count(self):
        ...
        estimate used to make sure enough continuation pages exist for the
        footer's own pagination to land on (see
    ...
    def _get_kuady_other_text(self, page_index):
        ...
        real_budget = self._get_kuady_footer_page_budget(page_index, total_pages)
